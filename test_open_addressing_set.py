...
def test_resize():
    ...
    s2 = s1.add(2)
    s3 = s2.add(3)
    # After adding 3 elements, load factor 3/4 > 0.7, should trigger
    # expansion, new capacity is 8
    ...
    s4 = s3.add(4)
    s5 = s4.add(5)
    # Adding the 6th element triggers expansion, capacity should be 16
    s6 = s5.add(6)
    assert s6.capacity == 16
    expected = [1, 2, 3, 4, 5, 6]
    assert sorted(s6.to_list()) == expected

    # Test 3: After removing elements and expanding again, the new set should
    # not contain removed elements
    s7 = s6.remove(3)
    s8 = s7.remove(5)