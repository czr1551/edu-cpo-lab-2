...
def test_resize():
    ...
    s = ImmutableOpenAddressingSet(initial_capacity=4,
                                   growth_factor=2)
    assert s.capacity == 4
    s1 = s.add(1)
    s2 = s1.add(2)
    s3 = s2.add(3)
    # After adding 3 elements, load factor 3/4 > 0.7,
    # should trigger resizing; new capacity becomes 8
    ...
    s4 = s3.add(4)
    s5 = s4.add(5)
    s6 = s5.add(6)  # Adding the 6th element triggers
    # resizing; capacity should be 16
    assert s6.capacity == 16
    expected = [1, 2, 3, 4, 5, 6]
    assert sorted(s6.to_list()) == expected

    # Test 3: After removing some elements, the new collection
    # should not include the deleted elements
    s7 = s6.remove(3)
    s8 = s7.remove(5)
    s9 = (s8.add(7).add(8).add(9).add(10).add(11)
          .add(12).add(13).add(14))
    # According to the original test expectation,
    # capacity should expand to 32
